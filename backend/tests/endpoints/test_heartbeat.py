...
def test_get_setup_library_info_returns_platforms(client, access_token):
    """Test get_setup_library_info returns existing platforms with rom counts"""
    with patch(
...
def test_get_setup_library_info_no_platforms(client, admin_user, access_token):
    """Test get_setup_library_info when no platforms exist"""
    with patch(
        "endpoints.heartbeat.fs_platform_handler.get_platforms"
    ) as mock_get_platforms:
...
def test_create_setup_platforms_success(client, admin_user, access_token):
    ...
    with patch(
        ...
        mock_add_platform.return_value = None
