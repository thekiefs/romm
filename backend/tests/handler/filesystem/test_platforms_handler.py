...
class TestFSPlatformsHandler:
    ...
    def test_get_platform_fs_structure_returns_slug(
        self, handler: FSPlatformsHandler, config
    ):
        """Test that get_platform_fs_structure returns the fs_slug directly"""
        fs_slug = "n64"
        with patch(
            "handler.filesystem.platforms_handler.cm.get_config", return_value=config
        ):
            result = handler.get_platform_fs_structure(fs_slug)
            assert result == fs_slug

    async def test_add_platform_creates_directory(
        self, handler: FSPlatformsHandler, config
    ):
        """Test that add_platform creates a directory named after the fs_slug"""
        fs_slug = "gba"
        with patch(
            "handler.filesystem.platforms_handler.cm.get_config", return_value=config
        ):
            with patch.object(handler, "make_directory") as mock_make_directory:
                ...
                mock_make_directory.assert_called_once_with(fs_slug)
    ...
    async def test_get_platforms_calls_list_directories_with_root_path(
        self, handler: FSPlatformsHandler, config
    ):
        """Test that get_platforms calls list_directories with empty path (library root)"""
        with patch(
            "handler.filesystem.platforms_handler.cm.get_config", return_value=config
        ):
            with patch.object(
